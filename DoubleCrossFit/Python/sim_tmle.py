...
file_path_to_save_results = "tmle_results"+str(setup)+".csv"  # file path to save all result output
...
for i in samples:
    ...
    tmle.treatment_model(g_model, g_estimator, bound=0.01)
    tmle.outcome_model(q_model, q_estimator)
    tmle.fit()
    bias.append(tmle.risk_difference - truth)
    stderr.append(tmle.risk_difference_se)
    lcl.append(tmle.risk_difference_ci[0])
    ucl.append(tmle.risk_difference_ci[1])
