...
#
# Developed by: Bonnie Shook-Sa (11.30.23)
###########################################################################
...
setwd("C:/Users/zivic/Documents/open-source/publications-code/GCompMSM")
actg <- as.data.frame(read.csv("actg.csv"))

# Creating categorical Karnofsky scores
actg$karnof90 <- ifelse(actg$karnof==90,1,0)
actg$karnof100 <- ifelse(actg$karnof==100,1,0)

# Labels for MSM results
MSM.labs <- c("intercept", "treat", "male", "treat*male")
...
out.model <- glm(cd4_20wk ~ treat + male + treat*male + idu + white + karnof90 +
                                    karnof100 + agec + age_rs1 + age_rs2 + age_rs3 +
                                    cd4c_0wk + cd4_rs1 + cd4_rs2 + cd4_rs3 +
                                    male*(cd4c_0wk + cd4_rs1 + cd4_rs2 + cd4_rs3),
                 data=actg)

#estimate potential outcomes for every participant
alltrt <- actg
alltrt$treat <- 1
alltrt$Ya <- predict(out.model, alltrt, type="response")

alluntrt <- actg
alluntrt$treat <- 0
alluntrt$Ya <- predict(out.model, alluntrt, type="response")

#stack data and fit MSM
stack <- rbind(alltrt,alluntrt)
MSM.GF <- glm(Ya ~ treat + male + treat*male, data=stack)
...
GF <- geex_GF(actg, cd4_20wk ~ treat + male + treat*male + idu + white + karnof90 +
                               karnof100 + agec + age_rs1 + age_rs2 + age_rs3 + 
                               cd4c_0wk + cd4_rs1 + cd4_rs2 + cd4_rs3 + 
                               male*(cd4c_0wk + cd4_rs1 + cd4_rs2 + cd4_rs3),
                               cd4_20wk ~ treat + male + treat*male, coef(MSM.GF))
...
GF.ests <- as.data.frame(cbind(as.vector(MSM.labs),
                               as.vector(GF.MSM.ests),
                               as.vector(GF.MSM.se)))
...
#propensity model
prop.model <- glm(treat ~ male + idu + white + karnof90 + karnof100 + agec +
                          age_rs1 + age_rs2 + age_rs3 + cd4c_0wk + cd4_rs1 +
                          cd4_rs2 + cd4_rs3,
                  family = binomial(), data = actg)
...
estfun_IPW <- function(data,models){   
  ...
  function(theta){
    ...
    c(e_scores(theta[e_pos]),       # scores from propensity model
      c(W*(Y-mu),                   # score from MSM intercept
        W*A*(Y-mu),                 # score from MSM A term
        W*V*(Y-mu),                 # score from MSM V term
        W*A*V*(Y-mu)))              # score from MSM A*V term
  }
}
...
#format output and compute CIs
IPW<-geex_IPW(actg, treat ~ male + idu + white + karnof90 + karnof100 + agec +
                            age_rs1 + age_rs2 + age_rs3 + cd4c_0wk + cd4_rs1 +
                            cd4_rs2 + cd4_rs3,
                            cd4_20wk ~ treat + male + treat*male, coef(MSM.IPTW))
...
IPW.ests <- as.data.frame(cbind(as.vector(MSM.labs),as.vector(IPW.MSM.ests),as.vector(IPW.MSM.se)))
